...
const UserController = {
    getUserPropose : async (req, res) => {
        ...
        try {
            ...
            const userFilter = getAllUser.filter((user) => {
                return !userCurrent.following.includes(user._id) && String(userCurrent._id) !== String(user._id)
            })
        res.status(200).json({
            status:'success',
            data: userFilter
    ...
    getUserFollowing: async (req, res) => {
        try {
            const userCurr = await UserModel.findOne(req.datatoken.id)
            const getFriendUserFollowing = await Promise( userCurr.following.map((user) => {
    ...
    updateUser: async (req, res) => {
        try {
            ...
            {
                res.status(500).json({status:'Bạn không thể chỉnh sửa người dùng này'})
            }
            else {
                ...
                const userUpdated = await UserModel.findByIdAndUpdate(req.params.id, {$set: req.body}, {new: true})
                res.status(200).json({
    ...
    followUser: async (req, res) => {
        try {
            ...
            {
                res.status(400).json({status: 'Bạn không thể theo dõi người dùng này'})
            }

            else {

            if (userCurrent.following.includes(req.params.id)) {
                userCurrent.following = userCurrent.following.filter((idUser) => idUser != req.params.id)
                userNotCurrent.follower = userNotCurrent.follower.filter((idUser) => idUser != req.datatoken.id)

                await UserModel.findByIdAndUpdate(req.params.id, {$set: userNotCurrent}, {new: true})
                await UserModel.findByIdAndUpdate(req.datatoken.id, {$set: userCurrent}, {new: true})
