...
const PostDetail = () => {
  ...
  useEffect(() => {
    const getApiOnePost = async () => {
      const dataApi = await fetch(
        `http://localhost:8080/bai-dang//bai-viet/${id}`,
        {
  ...
  useEffect(() => {
    const getApiDataCommentPost = async () => {
      const dataComment = await fetch(
        `http://localhost:8080/binh-luan/tat-ca-binh-luan/${id}`,
        {
          headers: { "Content-Type": "application/json" },
          method: "GET",
        }
  ...
  return (
    <div className="bg-gradient-to-b from-[#d1a6d1] to-[#f7e7f7]  h-screen px-5 py-16 bg-opacity-20">
      <div className=" flex">
        <div className="w-[65%]">
          <img
            className="w-[80%]"
            src={
              itemPost?.photo &&
              `http://localhost:8080/images/${itemPost?.photo}`
            }
            alt=""
          />
        </div>
        <div className="w-[35%]">
          ...
          >
            <img
              ...
              src={
                itemPost?.user?.profileimg
                  ? `http://localhost:8080/images/${itemPost?.user?.profileimg}`
            ...
            <div className="flex flex-col">
              <span className="font-semibold text-[19px]">
                {itemPost?.user?.username}
              </span>
              <span className="font-medium text-gray-700 text-[12px]">
                {moment(`${itemPost?.createdAt}`).format("YYYY-MM-DD HH:mm")}
              </span>
            </div>
          </Link>
          <div className="mt-3">
            <span className="text-[14px] font-normal flex items-center">
              <span>
                <MdLocationPin size={22}></MdLocationPin>
              </span>
            ...
            {showComment ? (
              <div>
                {commentPost?.length > 0 ? (
                  ...
                  <div>Bài viết này chưa có bình luận</div>
                )}
              </div>
            ) : (
              <div></div>
            )}
            <button onClick={() => setShowComment((state) => !state)}>
              {showComment ? "Ẩn bình luận" : "Xem bình luận"}
            </button>
            <div>
              <div className="my-5">
                <input
                  className="rounded-md w-[410px] h-[50px] outline-none px-2"
                  type="text"
                  value={textComment}
